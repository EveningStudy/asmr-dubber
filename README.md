...
接着选择处理内容、输出类型和成品组织。“仅生成字幕”会完成识别和必要的翻译，但不运行 TTS（语音合成）或混音；视频任务还可以勾选“让原声视频带字幕”，同时保留外部 SRT 和 LRC。纯音频不需要画面；视频任务会直接显示推荐图片的相对路径和预览，也可以改用作品中的其它图片或黑色背景。“每条音轨分别处理并输出”不会生成合并版。
