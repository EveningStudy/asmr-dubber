...
从[GitHub Releases](https://github.com/EveningStudy/asmr-dubber/releases/latest)下载 `ASMR-Dubber-windows-portable-v1.3.1.zip`，完整解压到最终使用位置。不要直接在压缩软件里运行。
...
设置页底部只有一个“保存设置”按钮。没有打开项目时，它保存以后新项目的默认值；已经打开项目时，它同时更新当前项目。
...
已有字幕或台本时，可以展开“使用已有台本或字幕”。SRT、VTT、ASS/SSA、LRC 默认使用文件中的时间轴；纯文本可以按长度估算，也可以先运行 ASR，再由大模型依据台本校正已有句子。
...
“工作台 → 批量处理”用于已经解压的音声作品目录。不需要逐个上传文件：输入包含音频的作品文件夹并扫描，程序会选出推荐的音频版本，再列出本次任务中的每条音轨。只有视频的作品请在“单个作品”中逐个处理。

![批量处理流程](assets/screenshots/batch-workflow.png)

- 勾选“包含特典、样本和 Free Talk”后，附加音轨会立即加入列表；
- 拖动卡片或使用上下按钮可以改变音轨顺序；
- SRT、VTT、ASS/SSA、LRC 和 TXT 会按音轨自动匹配，并判断为中文、日语或英语；
- 每条音轨都可以更换或关闭台本，也可以沿用字幕时间轴，或运行 ASR 重新定时、只采用台本文字。没有台本的音轨按正常 ASR 流程处理。

接着选择处理内容、输出类型和成品组织。“仅生成字幕”会完成识别和必要的翻译，但不运行 TTS（语音合成）或混音；所有任务都会保留外部 SRT 和 LRC，完成后可在批量页查看具体路径并打开输出目录。视频任务还可以勾选“让原声视频带字幕”。纯音频不需要画面；视频任务会直接显示推荐图片的相对路径和预览，也可以改用作品中的其它图片或黑色背景。“每条音轨分别处理并输出”不会生成合并版。

确认后加入队列，再继续扫描其它作品。队列支持拖动排序、编辑选项、移除和标记重新处理；修改任务不会影响队列中的其它作品。

选择“沿用文件时间轴”时，完整覆盖所选音轨的中文字幕会直接建立中文时间轴，不再运行 ASR 或翻译；日语或英语字幕跳过 ASR，之后仍会翻译。选择“运行 ASR 重新定时”时，程序以识别区间为准，按顺序映射台本文字；中文台本只翻译没有匹配上的句子。任务状态、日志和中间文件保存在 `.asmr-dubber/autoflow`，中断后可以继续。
