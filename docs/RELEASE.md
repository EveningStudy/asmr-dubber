...
ASMR Dubber 0.5.2 面向 64 位 Windows 10/11 和 x86_64 Linux。它处理日语音频或视频，提供
可校对的日语识别、中文翻译、音色克隆、混音和字幕工作流。

本版本修复纯中文台本无法删除句子的问题。把某一行的日文和中文都清空后，该句会正常从项目、
字幕和后续配音中移除，不再阻止保存或执行。

本版本增加中文台本导入。导入时可以选择日语台本或中文配音文本；中文 SRT 等字幕会保留原
